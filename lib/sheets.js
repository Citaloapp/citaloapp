...
export async function crearSolicitud(datos) {
  ...
  const row = [
    ...
    '',                          // R — payment_id (se escribe luego)
    datos.horarios || '',        // S — horarios de atención
  ];
...
export async function crearProfesionalActivo(datos) {
  ...
  const row = [
    ...
    datos.color_marca || '#0ea5e9',
    datos.obras_sociales || '',
    datos.duracion_turno_minutos || '30',
    'true',
    datos.email || '',
    datos.subscription_id || '',
  ];
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: 'profesionales!A:N',
...
export async function getProfesionalByEmail(email) {
  ...
  const row = data.find(r => {
    const obj = rowToObject(headers, r);
    return obj.profesional_email?.toLowerCase() === email && obj.activo?.toLowerCase() === 'true';
  });
  return row ? parseProfesional(rowToObject(headers, row)) : null;
}
