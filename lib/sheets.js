...
export async function getTurnosConfirmadosByFecha(slug, fecha) {
  ...
  const rows = res.data.values || [];
  const todos = turnosData(rows).map(r => rowToObject(TURNOS_COLS, r));

  // Log para detectar el formato real de fecha almacenado en Sheets
  const delProfesional = todos.filter(t => t.profesional_slug === slug);
  console.log('[sheets] fecha buscada:', fecha);
  console.log('[sheets] fechas almacenadas en Sheets para este slug:', delProfesional.map(t => `"${t.fecha}" (estado: ${t.estado})`));

  return delProfesional
    .filter(t => t.fecha === fecha && t.estado === 'confirmado')
    .map(t => t.hora);
}
