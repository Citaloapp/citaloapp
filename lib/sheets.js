...
export async function crearProfesionalActivo(datos) {
  ...
  const row = [
    ...
    datos.password || '',                // R: password (acceso al panel del profesional)
  ];
  await sheets.spreadsheets.values.append({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: 'profesionales!A:R',
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [row] },
  });
...
export async function verificarLoginProfesional(slug, password) {
  const sheets = getSheetsClient();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: 'profesionales!A:R',
  ...
  const activo = obj.activo?.toString().toLowerCase().trim() === 'true';
  return !!obj.password && obj.password === password && activo;
}

export async function actualizarPasswordProfesional(slug, password) {
  const sheets = getSheetsClient();
  const res = await sheets.spreadsheets.values.get({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: 'profesionales!A:A',
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: process.env.SPREADSHEET_ID,
    range: `profesionales!R${rowIndex + 1}`,
    valueInputOption: 'RAW',
